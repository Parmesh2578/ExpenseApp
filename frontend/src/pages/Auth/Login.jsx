...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      const { data } = await apiClient.post(AUTH.login, { email: trimmed, password })
      persistAuthTokens(
  ...
  return (
    <AuthLayout>
      <div className="lg:w-[70%] min-h-[calc(100vh-5rem)] md:min-h-0 md:h-full flex flex-col justify-center max-w-md w-full">
        <h3 className="text-2xl font-semibold text-slate-900 tracking-tight">Login</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full rounded-xl bg-primary py-2.5 text-sm font-semibold text-white shadow-sm shadow-violet-500/25 transition hover:brightness-105 active:scale-[0.99] disabled:opacity-60"
          >
