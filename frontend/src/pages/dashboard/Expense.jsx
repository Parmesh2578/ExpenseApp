...
import { currencyPresetLabel, filterMonth, sumExpense } from '../../utils/financeHelpers'
...
export default function Expense() {
  ...
  const monthTotal = useMemo(() => sumExpense(filterMonth(transactions)), [transactions])
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Track every purchase. This month:{' '}
          <span className="font-semibold text-primary">{formatMoneyFull(monthTotal)}</span>
        </p>
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        ...
        <p className="mt-0.5 text-xs text-slate-500">Bills, food, transport — keep categories consistent.</p>
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-6 py-4">
          <h3 className="text-sm font-semibold text-slate-900">This month&apos;s expenses</h3>
          {monthRows.length > 0 ? (
        ...
        <ul className="divide-y divide-slate-100">
          {monthRows.length === 0 ? (
            <li className="px-6 py-10 text-center text-sm text-slate-500">No expenses this month yet.</li>
          ) : (
            monthRows.map((t) => (
